...
#ifdef CONFIG_HUAWEI_KSTATE
static int get_euid_val(struct binder_proc *proc)
{
	const struct cred *cred = binder_get_cred(proc);
	if (cred != NULL)
		return cred->euid.val;
	return -1;
}
#endif
			       ...
			       binder_size_t extra_buffers_size)
{
	...
	if (reply) {
		...
		target_proc->tmp_ref++;
#ifdef CONFIG_HUAWEI_KSTATE
		cred_euid_val = get_euid_val(target_proc);
		if ((cred_euid_val >= 0) && (cred_euid_val <= 2000) &&
				...
				bool do_proc_work)
{
	...
	for (;;) {
		...
		if ((bt != NULL) && (bt->to_proc != NULL)) {
			cred_euid_val = get_euid_val(bt->to_proc);
			if ((!(bt->flags & TF_ONE_WAY)) &&
				((cred_euid_val >= 0) && (cred_euid_val <= 2000)) &&
