...
#define REGULATOR_1_2_MINUV 496000
#define REGULATOR_MINUV 1504000
#define REGULATOR_STEP 8000
#define REGULATOR_UV_MU 1000
...
static const struct regmap_config wl2868c_regmap_config = {
	...
	.reg_defaults = wl2868c_reg_defs,
	.num_reg_defaults = ARRAY_SIZE(wl2868c_reg_defs),
				...
				int max_uv)
{
	...
	mv = DIV_ROUND_UP(min_uv, REGULATOR_UV_MU); // uv to mv
	if (mv * REGULATOR_UV_MU > max_uv) {
	...
	if (wl2868c_reg->base == WL2868C_LDO1_VOUT || wl2868c_reg->base == WL2868C_LDO2_VOUT)
		vset_raw[0] = (min_uv-REGULATOR_1_2_MINUV)/REGULATOR_STEP;
	else
		vset_raw[0] = (min_uv-REGULATOR_MINUV)/REGULATOR_STEP;
...
static int wl2868c_regulator_get_voltage(struct regulator_dev *rdev)
{
	...
	if (wl2868c_reg->base == WL2868C_LDO1_VOUT || wl2868c_reg->base == WL2868C_LDO2_VOUT)
		return (((vset_raw[0])) * REGULATOR_STEP + REGULATOR_1_2_MINUV);
	else
		return (((vset_raw[0])) * REGULATOR_STEP + REGULATOR_MINUV);
}
...
static int wl2868c_regulator_init(struct wl2868c *chip, const char *name)
{
	...
	init_data = of_get_regulator_init_data(dev, reg_node, chip->rdesc);
	if (init_data == NULL) {
...
static int wl2868c_parse_regulator(struct regmap *regmap, struct device *dev)
{
	...
	struct wl2868c *wl2868c_reg = NULL;
