 ...
 *
 * Description: cam laser dev driver
 */
#include <sensors_class.h>
#include "vendor_laser_dev.h"
#include "vendor_laser_soc.h"
...
static int laser_data_remove(struct i2c_client *client)
{
	...
	if (!client) {
		CAM_ERR(CAM_SENSOR, "parameter error");
		return -EINVAL;
	}
...
static int cam_laser_i2c_driver_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	if (client == NULL || id == NULL) {
		CAM_ERR(CAM_SENSOR, "Invalid Args client: %pK id: %pK", client, id);
		return -EINVAL;
	}

	CAM_INFO(CAM_SENSOR, "I2C Address: %#04x\n", client->addr);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		CAM_ERR(CAM_SENSOR, "I2C check functionality failed");
		return -ENXIO;
	}
	...
	if (IS_ERR(vreg)) {
		CAM_ERR(CAM_SENSOR, "devm_regulator_get fail");
		goto soc_free;
	}

	rc = of_property_read_u32_array(o_ctrl->soc_info.dev->of_node, "rgltr-min-voltage",
		o_ctrl->soc_info.rgltr_min_volt, 1);
	if (!rc)
		rc = of_property_read_u32_array(o_ctrl->soc_info.dev->of_node, "rgltr-max-voltage",
			o_ctrl->soc_info.rgltr_max_volt, 1);

	if (!rc) {
		rc = regulator_set_voltage(vreg, o_ctrl->soc_info.rgltr_min_volt[0],
			o_ctrl->soc_info.rgltr_max_volt[0]);
		if (rc) {
			CAM_ERR(CAM_SENSOR, "regulator_set_voltage fail");
			goto soc_free;
		}
		CAM_INFO(CAM_SENSOR, "Minimum volatage value = %u, Maximum volatage value = %u",
			o_ctrl->soc_info.rgltr_min_volt[0], o_ctrl->soc_info.rgltr_max_volt[0]);
	}

	rc = of_property_read_u32_array(o_ctrl->soc_info.dev->of_node, "rgltr-load-current",
		o_ctrl->soc_info.rgltr_op_mode, 1);
	if (rc)
		o_ctrl->soc_info.rgltr_op_mode[0] = DEVICE_LOAD_CURRENT;

	CAM_INFO(CAM_SENSOR, "Load curent value = %u", o_ctrl->soc_info.rgltr_op_mode[0]);

	rc = regulator_set_load(vreg, o_ctrl->soc_info.rgltr_op_mode[0]);
	if (rc) {
		CAM_ERR(CAM_SENSOR, "regulator_set_load fail");
		goto soc_free;
	}

	rc = regulator_enable(vreg);
	if (rc) {
		CAM_ERR(CAM_SENSOR, "regulator_enable fail");
		goto soc_free;
	}

	if (&client->dev == NULL) {
		CAM_ERR(CAM_SENSOR, "&client->dev == NULL");
		goto soc_free;
	}
	dev = &client->dev;
	...
	if (IS_ERR_OR_NULL(pinctrl_state_laser_int)) {
		rc = -EINVAL;
		CAM_ERR(CAM_SENSOR, "pinctrl_lookup_state INT == NULL \n");
		goto err_pinctrl_select_state;
	}
	rc = pinctrl_select_state(pctrl, pinctrl_state_laser_int);
	if(rc) {
		rc = -EINVAL;
		CAM_ERR(CAM_SENSOR, "pinctrl_select_state INT == NULL \n");
		goto err_pinctrl_select_state;
	}
	...
	if (IS_ERR_OR_NULL(pinctrl_state_laser_en)) {
		rc = -EINVAL;
		CAM_ERR(CAM_SENSOR, "pinctrl_lookup_state EN == NULL \n");
		goto err_pinctrl_select_state;
	}
	rc = pinctrl_select_state(pctrl, pinctrl_state_laser_en);
	if(rc) {
		rc = -EINVAL;
		CAM_ERR(CAM_SENSOR, "INT pinctrl_select_state EN == NULL \n");
		goto err_pinctrl_select_state;
	}

	rc = cam_laser_driver_soc_init(o_ctrl);
	if (rc) {
		CAM_ERR(CAM_SENSOR, "failed: cam_sensor_parse_dt");
		goto regulator_err;
	}
	...
	if (rc) {
		CAM_ERR(CAM_SENSOR, "cell index %d probe fail", o_ctrl->soc_info.index);
		goto regulator_err;
	}

	CAM_INFO(CAM_SENSOR, "enter laser_probe");
...
static int cam_laser_i2c_driver_remove(struct i2c_client *client)
{
	...
	if (!o_ctrl) {
		CAM_ERR(CAM_SENSOR, "laser device is NULL");
		return -EINVAL;
	}

	CAM_INFO(CAM_SENSOR, "i2c driver remove invoked");

	rc = laser_data_remove(client);
	if (rc == 0) {
		CAM_INFO(CAM_SENSOR, "laser_data_remove success");
