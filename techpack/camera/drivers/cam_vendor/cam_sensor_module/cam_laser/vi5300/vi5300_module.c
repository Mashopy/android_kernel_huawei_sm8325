...
int vi5300_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	vi5300_data->wake_lock = wakeup_source_register(&client->dev, "laser_attach_wakelock");
	...
	/* match id */
	CAM_INFO(CAM_SENSOR, "vi5300 match id success.");
	set_hw_dev_flag(DEV_I2C_LASER);
	return 0;

exit_free_irq:
	free_irq(vi5300_data->irq, vi5300_data);
	wakeup_source_unregister(vi5300_data->wake_lock);
	mutex_destroy(&vi5300_data->work_mutex);
no_intr:
no_xshut:
	CAM_ERR(CAM_SENSOR, "vi5300_probe failed");
	kfree(vi5300_data);
	return ret;
}
