 ...
 *
 * Description: cam laser soc driver
 */
#include <linux/of.h>
...
int cam_laser_driver_soc_init(struct cam_laser_ctrl_t *o_ctrl)
{
	...
	if (!soc_info->dev) {
		CAM_ERR(CAM_SENSOR, "soc_info is not initialized");
		return -EINVAL;
	}

	of_node = soc_info->dev->of_node;
	if (!of_node) {
		CAM_ERR(CAM_SENSOR, "dev.of_node NULL");
		return -EINVAL;
	}
	...
	if (rc) {
		CAM_DBG(CAM_SENSOR, "No compatible string present for: %s",
			soc_info->dev_name);
		rc = 0;
	}
...
static int input_dev_open(struct input_dev *dev)
{
	...
	if (gpio_enable && (gpio_get_value(gpio_enable) == 0)) {
		...
		if (rc) {
			CAM_ERR(CAM_SENSOR, "Chip enable failed.\n");
			return -EIO;
		}
	}
	return rc;
...
int laser_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CAM_INFO(CAM_SENSOR, "laser_name is %s", laser_name);
	chip_data = kzalloc(sizeof(*chip_data), GFP_KERNEL);

	if (!chip_data) {
		CAM_ERR(CAM_SENSOR, "kzalloc failedv");
		rc = -ENOMEM;
		goto probe_failure;
	}
	...
	if (laser_input_dev == NULL) {
		CAM_ERR(CAM_SENSOR, "Error allocating input_dev");
	...
	if (rc) {
		CAM_ERR(CAM_SENSOR, "Error registering input_dev");
		goto input_reg_err;
	}
	...
	if (strcmp(laser_name, "tof8801") == 0) {
		...
		CAM_INFO(CAM_SENSOR, "tof8801 input_dev");
		...
		CAM_INFO(CAM_SENSOR, "stmvl53l1 input_dev");
		...
		CAM_INFO(CAM_SENSOR, "vi5300 input_dev");
	} else {
		CAM_ERR(CAM_SENSOR, "Error probed laser name");
		return -ENODEV;
	}

	rc = sensors_classdev_register(&laser_input_dev->dev, o_ctrl->class_dev);
	if (rc) {
		CAM_ERR(CAM_SENSOR, "Error registering sensor class");
		goto input_dev_alloc_err;
	}

	rc = sysfs_create_groups(&laser_input_dev->dev.kobj, o_ctrl->laser_attr_group);
	if (rc) {
		CAM_ERR(CAM_SENSOR, "Error creating sysfs attribute group");
...
int laser_remove(struct i2c_client *client)
{
	...
	if (!client) {
		CAM_ERR(CAM_SENSOR, "input client is NULL");
		return -EINVAL;
	}
