...
void updata_dev_msg(struct dev_msg_t *dev)
{
	...
	dev_node->state = -1;
	strncpy_s(dev_node->dev_name, DEV_NAME_MAX_LENGTH,
...
static long debug_ioctl(struct file *fd, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_DEBUG_MSG: {
		...
		if (copy_to_user((void __user *)arg, &send_data,
			_IOC_SIZE(IOCTL_DEBUG_MSG))) {
...
module_init(camera_debug_init);
module_exit(camera_debug_exit);
#endif
