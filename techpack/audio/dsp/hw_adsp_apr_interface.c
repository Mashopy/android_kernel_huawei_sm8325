...
#ifdef TEST_CAL_ENABLE
struct test_cal_block {
	struct list_head list;
	struct rtac_cal_block_data cal;
};
#endif /* TEST_CAL_ENABLE */
...
#ifdef TEST_CAL_ENABLE
	struct list_head test_cal_list;
...
#ifdef TEST_CAL_ENABLE
static int test_cal_map(struct hw_afe_ctl *afe_ctl, uint32_t map_size)
{
	int ret;
	size_t len;
	struct test_cal_block *tblock = NULL;
	struct rtac_cal_block_data *cal = NULL;

	tblock = kzalloc(sizeof(*tblock), GFP_KERNEL);
	if (!tblock)
		return -ENOMEM;

	INIT_LIST_HEAD(&tblock->list);
	cal = &tblock->cal;

	/* Minimal chunk size is 4K */
	cal->map_data.map_size = map_size;
	...
	pr_info("%s: paddr 0x%pK, kvaddr 0x%pK, map_size 0x%x\n",
	...
	list_add_tail(&tblock->list, &afe_ctl->test_cal_list);
	return 0;
...
static int test_cal_print(struct hw_afe_ctl *afe_ctl)
{
	struct test_cal_block *tblock = NULL;
	struct test_cal_block *temp = NULL;
	struct rtac_cal_block_data *cal = NULL;

	list_for_each_entry_safe(tblock, temp, &afe_ctl->test_cal_list, list) {
		cal = &tblock->cal;
		pr_info("%s: paddr 0x%pK, kvaddr 0x%pK, map_size 0x%x\n",
			__func__, &cal->cal_data.paddr,
			cal->cal_data.kvaddr, cal->map_data.map_size);
...
static int test_cal_unmap(struct hw_afe_ctl *afe_ctl)
{
	int ret;
	struct test_cal_block *tblock = NULL;
	struct test_cal_block *temp = NULL;
	...
	list_for_each_entry_safe(tblock, temp, &afe_ctl->test_cal_list, list) {
		...
		list_del_init(&tblock->list);
		if (cal->map_data.map_handle) {
			ret = afe_unmap_rtac_block(&cal->map_data.map_handle);
			if (ret)
				pr_err("%s: unmap buffer failed! ret = %d\n",
					__func__, ret);
		}

		if (cal->map_data.dma_buf) {
			msm_audio_ion_free(cal->map_data.dma_buf);
			cal->map_data.dma_buf = NULL;
		}
...
static int test_cal_set(void *data, u64 val)
{
	int ret = -EINVAL;
	...
	if (val <= 10000) /* map */
		ret = test_cal_map(&g_hw_afe, SZ_4K * val);
	else if (val == 10001) /* unmap */
		ret = test_cal_unmap(&g_hw_afe);
	else if (val == 10002) /* print */
		ret = test_cal_print(&g_hw_afe);

	return ret;
}
DEFINE_DEBUGFS_ATTRIBUTE(test_cal_fops, NULL,
	test_cal_set, "%llu\n");

static void test_cal_init(struct hw_afe_ctl *afe_ctl)
{
	INIT_LIST_HEAD(&afe_ctl->test_cal_list);
	afe_ctl->test_entry = debugfs_create_file("adsp_test_cal", 0664,
		NULL, NULL, &test_cal_fops);
}

static void test_cal_deinit(struct hw_afe_ctl *afe_ctl)
{
	test_cal_unmap(afe_ctl);
	debugfs_remove(afe_ctl->test_entry);
}
#endif /* TEST_CAL_ENABLE */
